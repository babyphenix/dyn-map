(function($) {
    ...
    $.fn.conceptMap = function(options) {
        ...
        /*
         * CONCEPT
         * define of node content
         */

        function createConcept(title, x, y) {
            var id = Math.round(Math.random() * 100);
            var content =
                "<div id='" + id + "' class='concept' data-container='body' data-toggle='popover' data-placement='right' data-content='ノード詳細情報の表示想定。'>" + title +
                "   <div class='btn-group'>" +
                ...
                "   </div>" +
                "</div>"; //NOD-003
            var html = $(content); //$("<div class='concept'>" + title + "</div>");
            html.popover(); //NOD-004
            var obj = {
                title: title,
        ...
        function createRelation(from, to, label) {
            ...
            obj.getEdgePoints = function() {
                if (!caches.edgePointCache[this.title]) {
                    ...
                    caches.edgePointCache[this.title] = {
                        p1: p1,
                        p2: p2,
                        theta: theta
                    };
        ...
        var conceptMap = {
            ...
            drawRelations: function() {
                ...
                //ctx.lineWidth = 2;
                // //LNK-001
                // var lineColorArray = ["#E74C3C","#3498DB"];
                // var _lineColor = lineColorArray[Math.round(Math.random()*10)%2];

                // Math.round(Math.random()*10)%2
                // ctx.fillStyle = _lineColor;//"rgb(100, 100, 100)";//LNK-001
                // ctx.strokeStyle = _lineColor;//"rgb(100, 100, 100)";//LNK-001
                ...
                for (var k in this.relations) {
                    ...
                    //LNK-001
                    var lineColorArray = ["#33B5E5", "#FF4444", "#E5E5E5"];
                    var _lineColor = lineColorArray[Math.round(Math.random() * 100) % 3];
                    ctx.fillStyle = _lineColor; //"rgb(100, 100, 100)";//LNK-001
                    ctx.strokeStyle = _lineColor; //"rgb(100, 100, 100)";//LNK-001
                    //LNK-002
                    ctx.lineWidth = (Math.round(Math.random() * 100) % 2) * 2 + 1;
                    ...
                    //LNK-004
                    var _dLeft = (edgePoints.p1.x - edgePoints.p2.x) / 4;
                    var _dTop = (edgePoints.p1.y - edgePoints.p2.y) / 4;
                    console.log(_dLeft + "   " + _dTop);

                    var x = edgePoints.p2.x + opts.ARROW_SIZE * Math.cos(edgePoints.theta + Math.PI - opts.ARROW_ANGLE) + _dLeft;
                    var y = edgePoints.p2.y + opts.ARROW_SIZE * Math.sin(edgePoints.theta + Math.PI - opts.ARROW_ANGLE) + _dTop;
                    ctx.moveTo(x, y);
                    ctx.lineTo(edgePoints.p2.x + _dLeft, edgePoints.p2.y + _dTop);

                    x = edgePoints.p2.x + opts.ARROW_SIZE * Math.cos(edgePoints.theta + Math.PI + opts.ARROW_ANGLE) + _dLeft;
                    y = edgePoints.p2.y + opts.ARROW_SIZE * Math.sin(edgePoints.theta + Math.PI + opts.ARROW_ANGLE) + _dTop;
                    ctx.lineTo(x, y);
